    ...
    <div className="flex items-center justify-between gap-3 rounded-2xl bg-slate-800/60 px-4 py-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="w-24 text-center text-2xl font-extrabold tabular-nums">
          {value}<span className="text-base font-bold text-slate-400">{unit}</span>
        </div>
        <button
...
function HomeInner() {
  ...
  const effective = (e: { id: string; workSeconds: number; restSeconds: number }) => ({
    work: pacing.exercises[e.id]?.workSeconds ?? (pacing.customEnabled && !fast ? pacing.workSeconds : e.workSeconds),
    rest: pacing.exercises[e.id]?.restSeconds ?? (pacing.customEnabled && !fast ? pacing.restSeconds : e.restSeconds),
  });
        ...
        <div className="mt-8 grid grid-cols-2 gap-10 max-lg:grid-cols-1">
          <div>
            ...
            <p className="mt-4 text-lg text-slate-400">
              Follow-along player. Big timers, loud beeps, no touching the screen mid-set.
              Space = pause • F = finish • N = skip • V = voice.
            </p>
            ...
            <div className="mt-6 rounded-3xl border border-green-900/60 bg-green-950/30 p-5">
              ...
              <ul className="list-disc space-y-1 pl-5 text-lg text-slate-200">
                <li>Stop each set <strong>1–2 reps before failure</strong> — leave a rep in the tank.</li>
                <li>Aim for <strong>1.2–1.6 g protein per kg</strong> body weight daily, spread across meals.</li>
                <li>Train <strong>2–3× per week</strong>, covering all major muscle groups.</li>
              </ul>
              <p className="mt-2 text-sm text-slate-500">
                General fitness information, not medical advice. If you take GLP-1 medication or have
                a health condition, check with your clinician before training.
              </p>
            </div>
            ...
            <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/40 p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <button
                  onClick={() => updatePacing({ customEnabled: !pacing.customEnabled })}
              ...
              {pacing.customEnabled ? (
                <div className="space-y-2">
                  <Stepper label="Get ready" value={pacing.readySeconds} unit="s"
                    min={PACE_LIMITS.ready.min} max={PACE_LIMITS.ready.max} step={PACE_LIMITS.ready.step}
                    onChange={(v) => updatePacing({ readySeconds: v })} />
                  <Stepper label="Work" value={pacing.workSeconds} unit="s"
                    min={PACE_LIMITS.work.min} max={PACE_LIMITS.work.max} step={PACE_LIMITS.work.step}
                    onChange={(v) => updatePacing({ workSeconds: v })} />
                  <Stepper label="Rest" value={pacing.restSeconds} unit="s"
                    min={PACE_LIMITS.rest.min} max={PACE_LIMITS.rest.max} step={PACE_LIMITS.rest.step}
                    onChange={(v) => updatePacing({ restSeconds: v })} />
                  <div className="flex items-center justify-between pt-1">
                    <p className="text-base text-slate-400">Applies to every set. Saved on this device.</p>
                    <button
                      onClick={() => updatePacing({ ...DEFAULT_PACING, customEnabled: true })}
                ...
                <p className="text-lg text-slate-400">
                  Programmed pace: 40s work • 90s rest on compounds, 60s on isolations • 5s get-ready. Turn Custom on to scale the day up or down, or tap a move to tune it alone.
                </p>
              )}
            </div>
          </div>
          <div className="rounded-3xl border border-slate-800 bg-slate-900/40 p-4">
            <div className="mb-2 text-xl font-bold uppercase tracking-wider text-slate-400">Today&apos;s lineup — tap a move to tune its pace</div>
            <ul className="space-y-2">
              {workout.exercises.map((e, i) => {
                ...
                return (
                  <li key={e.id} className={`rounded-2xl p-3 ${tuned ? "bg-sky-900/40" : "bg-slate-800/60"}`}>
                    <button
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="text-2xl font-bold leading-tight">
                          {e.name}
                          {tuned && <span className="ml-2 rounded-lg bg-sky-500/25 px-2 py-0.5 align-middle text-sm font-bold text-sky-300">TUNED</span>}
                        </div>
                        <div className="text-lg text-slate-400">
                          {e.id === "one-arm-row" ? "6 sets (3/side)" : `${e.sets} sets`} × {e.targetReps} reps • {eff.work}s work • {eff.rest}s rest
                        </div>
                      </div>
                    ...
                    {open && (
                      <div className="mt-2 space-y-2 pl-14">
                        ...
                        <Stepper label="Rest" value={eff.rest} unit="s"
                          min={PACE_LIMITS.rest.min} max={PACE_LIMITS.rest.max} step={PACE_LIMITS.rest.step}
                          onChange={(v) => updateExercisePacing(e.id, { restSeconds: v })} />
                        {tuned && (
                          <button
...
export default function Page() {
  const fallback = useMemo(() => <div className="p-10 text-2xl">Loading…</div>, []);
  return (
